...
def altar_info(player):
    ...
    mess = 'Оставьте дары для Хранителя Подземелья!\n' + \
           ...
           'Ник - 5' + icon('skull') + '\n' + \
           'Атака - 5' + icon('skull') + '\n' + \
           'Атака сквозь щит - 10' + icon('skull') + '\n' + \
    ...
    mess += '\nКоманды 💀 Алтаря 💀:\n' + \
            ...
            'Алтарь атака [ID или ссылка на игрока] - атака игрока независимо от наличия щита и перезарядки атаки\n' + \
            'Алтарь разведка [ID или ссылка на игрока] - даёт полную информацию об игроке\n' + \
            'Алтарь пещеры - сброс перезарядки пещер'

    return mess


def altar(command, player, action_time, chat_info):
    part = command.split()
    if len(part) < 2:
        mess = 'Команды 💀 Алтаря 💀:\n' + \
               'Алтарь - стоимость\n' + \
    ...
    # ПЕЩЕРЫ
        ...
        if action_time >= player.cave_progress.time:
            return 'Вы и так можете отправиться в пещеры!'
        if player.cave_progress:
            player.cave_progress.time = 0
            player.build.stock.skull -= 5
            player.build.stock.save(update_fields=['skull'])
            player.cave_progress.save(update_fields=['time'])
            mess = 'Хранитель Подземелья восстановил вам силы для исследования пещер!'
            return mess
